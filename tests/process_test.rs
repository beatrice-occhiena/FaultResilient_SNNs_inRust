...
#[test]
fn test_process_snn_one_layer_dt_changed() {
    let dt = 0.01;
    let snn = SNNBuilder::new(2)
        .add_layer(vec![
            Lif::new(0.1, 0.05, 0.3, 1.0, dt),
            Lif::new(0.1, 0.05, 0.3, 1.0, dt),
            Lif::new(0.1, 0.05, 0.3, 1.0, dt)], vec![
            vec![0.1, 0.2],
            vec![0.3, 0.4],
            vec![0.5, 0.6]], vec![
            vec![0.0, -0.1, -0.15],
...
fn test_snn_wrong_input_spikes() {
    let snn = SNNBuilder::new(2)
        .add_layer(vec![
            Lif::new(0.84, 0.05, 0.3, 1.0, 1.0),
            Lif::new(0.12, 0.87, 0.3, 0.89, 1.0)], vec![
            vec![0.12, 0.5],
            vec![0.53, 0.43]], vec![
            vec![0.0, -0.3],
            vec![-0.4, 0.0]
